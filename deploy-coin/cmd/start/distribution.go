...
func makeDistributionTx(d *daemon.Daemon) coin.Transaction {
	var tx coin.Transaction

	blocks, err := d.Visor.GetSignedBlocksSince(0, 1)
	fmt.Println(blocks, err)

	/*
		tx.PushInput(gb.GetTransaction())

		for i := range distAddr {
			addr := cipher.MustDecodeBase58Address(distAddr[i])
